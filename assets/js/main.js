document.addEventListener('DOMContentLoaded', function () {
  ...
  if (toggle && overlay) {
    ...
    window.addEventListener('resize', function () {
      if (window.innerWidth >= 992 && overlay.classList.contains('show')) closeNav();
    });
  }
    ...
    pills.forEach(function (pill) {
      pill.addEventListener('click', function () {
        ...
        pkgCards.forEach(function (card) {
          var show = cat === 'semua' || card.getAttribute('data-category') === cat;
  ...
  if (form) {
    form.addEventListener('submit', function (e) {
      ...
      var text = 'Halo Gemilang Katun Outbound, saya ingin bertanya:%0A'
        + 'Nama: ' + nama + '%0A'
